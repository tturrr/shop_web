 ...
 //$_POST['bno']이 있을 때만 $bno 선언
  ...
  if(isset($bNo)) {
    ...
    if($row['cnt']) {
    		...
    		$sql = 'UPDATE women_board set b_brand="' . $bBrand .'" ,b_price="' . $bPrice .'" ,b_image="' . $bImage .'",  b_title="' . $bTitle . '", b_content="' . $bContent . '" where b_no = ' . $bNo;
  	...
  	$sql = "INSERT INTO women_board (b_no, b_title, b_content, b_price, b_hit, b_brand, b_image)
...
if(empty($msg)) {
    ...
    if(empty($bNo)) {
		...
		$replaceURL = './womens_details_modify.php?bno=' . $bNo;
